...
const config = {
  ...
  gpu: {
    // 'none' (default, CPU only) | 'auto' | 'nvidia' | 'intel' | 'amd' | 'mac'
    mode: process.env.GPU_ACCELERATION || 'none',
  },

  repliz: {
    baseUrl: process.env.REPLIZ_BASE_URL || 'https://api.repliz.com',
    accessKey: process.env.REPLIZ_ACCESS_KEY || null,
    secretKey: process.env.REPLIZ_SECRET_KEY || null,
    enabled: Boolean(process.env.REPLIZ_ACCESS_KEY && process.env.REPLIZ_SECRET_KEY),
  },
};
