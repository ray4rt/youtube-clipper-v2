...
/**
 * Menjalankan command builder dengan GPU terlebih dahulu (jika tersedia/di-enable),
 * dan otomatis fallback ke CPU (libx264) sekali jika proses GPU gagal di tengah jalan
 * (mis. driver bermasalah, VRAM penuh, sesi encoder NVENC melebihi limit).
 *
 * @param {(useGpu: boolean) => { command: import('fluent-ffmpeg').FfmpegCommand, usedGpu: boolean }} buildCommand
 * @param {string} outputPath
 * @param {(percent: number) => void} [onProgress]
 * @returns {Promise<string>}
 */
function runWithGpuFallback(buildCommand, outputPath, onProgress) {
  const attempt = (useGpu) => {
    const { command, usedGpu } = buildCommand(useGpu);
    return runCommand(command, outputPath, onProgress).catch((err) => {
      if (usedGpu) {
        logger.warn('Encoding via GPU gagal, otomatis fallback ke CPU (libx264).', {
          error: err.message,
        });
...
/**
 * Terapkan opsi codec video (GPU atau CPU) ke command fluent-ffmpeg.
 * @param {import('fluent-ffmpeg').FfmpegCommand} command
 * @param {boolean} useGpu
 * @param {number} [crf=23] - Dipetakan ke parameter kualitas setara di tiap encoder GPU
 * @returns {boolean} usedGpu
 */
function applyVideoCodec(command, useGpu, crf = 23) {
  const gpu = useGpu ? gpuService.detectGpuEncoder() : null;

  if (gpu) {
    if (gpu.config.hwaccelInput && gpu.config.hwaccelInput.length) {
      command.inputOptions(gpu.config.hwaccelInput);
    }
    command.videoCodec(gpu.config.videoCodec);
    command.outputOptions(gpu.config.buildOutputOptions(crf));
    return true;
  }

  command.videoCodec('libx264');
  command.outputOptions([`-crf ${crf}`, '-preset veryfast', '-movflags +faststart']);
  return false;
}

/**
 * Memotong video (clip) sesuai rentang waktu, opsional resize resolusi.
 * Menggunakan GPU hardware encoder jika GPU_ACCELERATION diaktifkan di .env,
 * dengan fallback otomatis ke CPU jika GPU gagal.
 * @param {object} params
...
function clipVideo({ inputPath, outputPath, startSeconds, durationSeconds, resolution, onProgress }) {
  ...
  const buildCommand = (useGpu) => {
    const command = ffmpeg(inputPath).setStartTime(startSeconds).setDuration(durationSeconds);
    const usedGpu = applyVideoCodec(command, useGpu, 23);
    ...
    return { command, usedGpu };
  };

  return runWithGpuFallback(buildCommand, outputPath, onProgress);
}
...
function compressVideo(inputPath, outputPath, crf = 28, onProgress) {
  const buildCommand = (useGpu) => {
    const command = ffmpeg(inputPath);
    const usedGpu = applyVideoCodec(command, useGpu, crf);
    command.audioCodec('aac');
    return { command, usedGpu };
  };
...
/**
 * Status GPU acceleration saat ini, dipakai untuk health check / info di frontend.
 * @returns {{ enabled: boolean, vendor: string|null, codec: string|null }}
 */
function getGpuStatus() {
  const gpu = gpuService.detectGpuEncoder();
  return {
    enabled: Boolean(gpu),
    vendor: gpu ? gpu.vendor : null,
    codec: gpu ? gpu.config.videoCodec : null,
  };
}
